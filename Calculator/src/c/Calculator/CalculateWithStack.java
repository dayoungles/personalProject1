...
public class CalculateWithStack {
	...
	/**
	 * if 문의 operatorStack.peek() <= priorityNum이면 답이 다르다. 왜???
	 * 
	 * @param sample
	 * @param priorityNum
	 */
	private static void stopoverCalculate(CalculateWithStack sample,
			int priorityNum) {
		while (!operatorStack.isEmpty()) {
			if (operatorStack.peek() < priorityNum)
				break;
			else {
				float middleProcess = sample.calculate(operatorStack.pop(),
	...
	// private static void stopoverCalculate(CalculateWithStack sample, int
	// priorityNum) { // 우선순위 비교해서 계산까지.종료조건!!!!이건 비교만 하지, 쌓여있을 때는 어떻게 할지?
	// if (operatorStack.isEmpty()) {
	// operatorStack.push(priorityNum);
	// } else {
	// while(true/*isEmpty*/){//스택 픽이랑 비교해서 크거나 같을 때까지만 돈다.
	// if (priorityNum >= operatorStack.peek()) { //우선순위가 높으면 일단 operatorStack에
	// 쌓는다.
	// operatorStack.push(priorityNum);
	//
	// } else {
	...
	/*
	 * public void bracketCalc(){ 그러면 priorityNum == 6이랑 7이 나왔을 때 예외상황을 설정해야..
	 * 6이 들어오면 이 메소드로 옮겨오고, 이 안에서 또 stopoverCalc를 이용해서 나온 값은 변수에 저장해 둔다... 그러다가,
	 * 7이 들어오면 변수에 저장한 값을 fStack에 푸쉬..
	 * 
	 * if (priorityNum == 6){//6이면 7이 들어올 때까지...while? float inBracketCal =
	 * sample.calculate(operatorStack.pop(), fStack.pop(), fStack.pop()); }
	 */
}
